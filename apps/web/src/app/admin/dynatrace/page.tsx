...
import { api, type DynatraceConnection, type DynatraceSignal, type DynatraceSignalReport, type OrgInfo } from '../../../lib/api';
...
export default function DynatracePage() {
  ...
  function load() {
    ...
    Promise.all([
      ...
      .catch((e: { status?: number; message?: string }) => {
        ...
        else setError(e?.message ?? 'Failed to load Dynatrace connections');
      })
  ...
  function openAdd() {
    router.push('/admin/dynatrace/new');
  }

  function openEdit(c: DynatraceConnection) {
    resetForm();
    setName(c.name);
    setEnvironmentUrl(c.environmentUrl);
  ...
  async function save() {
    ...
    setBusy(true);
    try {
      const body: Record<string, unknown> = { name: name.trim(), environmentUrl };
      if (apiToken.trim()) body.apiToken = apiToken;
      const res = await fetch(`/api/integrations/dynatrace/connections/${panel.id}`, {
      ...
      const { connection } = (await res.json()) as { connection: DynatraceConnection };
  ...
  async function verify(c: DynatraceConnection) {
    ...
    try {
      const res = await fetch(`/api/integrations/dynatrace/connections/${c.id}/verify`, {
      ...
      const { report } = (await res.json()) as { report: DynatraceSignalReport; availableSignals: string[] };
  ...
  async function remove(c: DynatraceConnection) {
    if (!window.confirm('Remove this connection? Project scopes that use it will stop working.')) return;
    setRemovingId(c.id);
    setError('');
    try {
      await api(`/api/integrations/dynatrace/connections/${c.id}`, { method: 'DELETE' });
  ...
  if (loading) return <WorkspaceShell org={org}><PageHeader title="Dynatrace" /><Skeleton rows={4} /></WorkspaceShell>;
  if (unauthorized) return (
    <WorkspaceShell org={org}><PageHeader title="Dynatrace" />
  ...
  return (
    <WorkspaceShell org={org}>
      <PageHeader title="Dynatrace" actions={<Button onClick={openAdd}>Add connection</Button>} />
        ...
        <Card className="mb-4">
          ...
          <div className="mt-4 flex flex-col gap-4">
            ...
            <DynatraceCredsForm
              ...
              editing
            />
            <p className="text-xs text-fg-faint">Leave the token field blank to keep the current secret.</p>
