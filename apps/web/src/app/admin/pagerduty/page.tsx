...
import { api, type PagerDutyConnection, type PagerDutySignal, type PagerDutySignalReport, type PagerDutyRegion, type OrgInfo } from '../../../lib/api';
...
export default function PagerDutyPage() {
  ...
  function load() {
    ...
    Promise.all([
      ...
      .catch((e: { status?: number; message?: string }) => {
        ...
        else setError(e?.message ?? 'Failed to load PagerDuty connections');
      })
  ...
  function openAdd() {
    router.push('/admin/pagerduty/new');
  }

  function openEdit(c: PagerDutyConnection) {
    resetForm();
    setName(c.name);
    setRegion(c.region);
  ...
  async function save() {
    ...
    setBusy(true);
    try {
      const body: Record<string, unknown> = { name: name.trim(), region };
      if (apiToken.trim()) body.apiToken = apiToken;
      const res = await fetch(`/api/integrations/pagerduty/connections/${panel.id}`, {
      ...
      const { connection } = (await res.json()) as { connection: PagerDutyConnection };
  ...
  async function verify(c: PagerDutyConnection) {
    ...
    try {
      const res = await fetch(`/api/integrations/pagerduty/connections/${c.id}/verify`, {
      ...
      const { report } = (await res.json()) as { report: PagerDutySignalReport; availableSignals: string[] };
  ...
  async function remove(c: PagerDutyConnection) {
    if (!window.confirm('Remove this connection? Project scopes that use it will stop working.')) return;
    setRemovingId(c.id);
    setError('');
    try {
      await api(`/api/integrations/pagerduty/connections/${c.id}`, { method: 'DELETE' });
  ...
  if (loading) return <WorkspaceShell org={org}><PageHeader title="PagerDuty" /><Skeleton rows={4} /></WorkspaceShell>;
  if (unauthorized) return (
    <WorkspaceShell org={org}><PageHeader title="PagerDuty" />
  ...
  return (
    <WorkspaceShell org={org}>
      <PageHeader title="PagerDuty" actions={<Button onClick={openAdd}>Add connection</Button>} />
      <p className="mb-4 text-sm text-fg-muted">
        Reusable read-only credentials. Projects pick a connection and scope it to specific services and teams.
      </p>
      ...
      {panel && (
        <Card className="mb-4">
          ...
          <div className="mt-4 flex flex-col gap-4">
            ...
            <p className="text-xs text-fg-faint">Leave the API key blank to keep the current secret.</p>
