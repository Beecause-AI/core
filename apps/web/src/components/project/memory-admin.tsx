...
import { fetchMemories, addMemory, deleteMemory, type AgentMemory } from '../../lib/api';
...
export function MemoryAdmin({ slug }: { slug: string }) {
  ...
  async function handleAdd() {
    ...
    setError('');
    try {
      const mem = await addMemory(slug, trimmed);
  ...
  return (
    <section className="flex flex-col gap-6">
      <div className="flex flex-col gap-4">
        <h2 className="text-base font-semibold text-fg">Team memory</h2>
      ...
      {memories === null ? (
        ...
        <EmptyState
          title="No team memories yet"
          body="Add memories above so assistants can recall them during incidents."
        />
      ) : (
