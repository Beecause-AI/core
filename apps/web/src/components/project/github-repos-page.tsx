...
const PROVIDER = '/api/org/integrations/github/catalog';

function syncedLabel(sync: CatalogSync): string {
  if (sync.status === 'syncing') return `Syncing from GitHub… ${sync.repoCount} found`;
...
export function GithubReposPage({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([res, scope, settings]) => {
        ...
        setIssuesOrgEnabled(settings.issuesOrgEnabled);
        if (res.sync.status === 'syncing' || res.sync.stale) void poll();
      })
      .catch((e: { status?: number; message?: string }) => {
        if (e?.status === 401) return;
        setSelected([]);
        setError(e?.status === 422 ? 'Connect GitHub under Admin → Integrations first.' : (e?.message ?? 'Failed to load'));
      });
  ...
  async function add(repoFullName: string) {
    ...
    try {
      const repo = await api<ProjectRepo>(`/api/org/projects/${slug}/repos`, { method: 'POST', body: JSON.stringify({ repoFullName }) });
  ...
  async function remove(repo: ProjectRepo) {
    ...
    try {
      await api<void>(`/api/org/projects/${slug}/repos/${repo.id}`, { method: 'DELETE' });
  ...
  return (
    <div className="flex flex-col gap-8">
      <h2 className="text-lg font-semibold text-fg">GitHub repositories</h2>
      ...
      <section className="flex flex-col gap-3">
        <div className="flex flex-col gap-1">
          <h3 className="text-sm font-semibold text-fg">GitHub issues</h3>
          <p className="text-sm text-fg-muted">Let the Slack assistant offer to raise a GitHub issue after a fixable RCA.</p>
        </div>

        <div className="rounded-card border border-edge bg-surface px-5 py-4 flex flex-col gap-3">
          {/* GitHub issue creation (project opt-in; gated by the org master switch). */}
          <div className="flex items-start justify-between gap-4">
            <div className="flex flex-col">
              <span className={`text-sm font-medium${issuesOrgEnabled ? '' : ' opacity-40'}`}>GitHub issue creation</span>
          ...
          {!issuesOrgEnabled && (
            <p className="text-xs text-fg-faint">Org master switch is off — an org admin must enable GitHub issue creation under Admin → GitHub first.</p>
          )}
      ...
      <section className="flex flex-col gap-3">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-sm text-fg-muted">Search the repositories the GitHub connection can see.</p>
        </div>
        ...
        {!data ? <Skeleton rows={6} /> : rows.length === 0 ? (
          manualValid && !exactShown ? (
            ...
            <EmptyState title="No matching repositories" body="Type a full owner/name to add one directly." />
          )
        ) : (
          <>
            <div className="divide-y divide-edge rounded-card border border-edge bg-surface">
